...
#net = torch.load('C:\\projects\\STALSTM\\models\\manual_label_all_e60.pth') # model trained using all data
net = torch.load('C:\\projects\\STALSTM\\models\\adult_train_test_split_oct_2024_finetune.pth') # model trained using patient from 20-80 from KKH and SGH
...
IN_DIM = 956 * 30  # always use 60 / base number to get the final datapoints, e.g. 60 / base_2 = 30
...
#dp = data_preprocess(file_path='C:\\projects\\STALSTM\\dataset\\manual combined SGH_KKH\\training_data_oct_2024.csv',
#                     train_per=TRAIN_PER, vali_per=VALI_PER, in_dim=IN_DIM)
dp = data_preprocess(file_path='C:\\projects\\STALSTM\\dataset\\manual combined SGH_KKH\\testing_data_oct_2024_no_stride.csv',
                     train_per=TRAIN_PER, vali_per=VALI_PER, in_dim=IN_DIM)
...
test_dataloader = torch.utils.data.DataLoader(train_data_trans,
                                              batch_size=BATCH_SIZE,
...
def test():
    ...
    np.random.seed(0)
    true_labels = np.random.randint(0, 2, size=100)  # Binary true labels (0 or 1)
    predicted_labels = np.random.randint(0, 2, size=100)  # Random predicted labels (0 or 1)

    # Convert predicted labels to scores
    predicted_scores = predicted_labels  # Assuming predicted labels as scores (0 or 1)
...
def main():
    ...
    #torch.save(net, 'C:\\projects\\STALSTM\\models\\manual_label_all_e70.pth')
