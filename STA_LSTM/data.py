...
class data_preprocess(object):
    ...
    def load_data(self):
        ...
        #raw_data = pd.read_csv(self.file_path, header = None).values
        
        df = pd.read_csv(self.file_path, header = None) #chookw
        df1 = df.iloc[np.random.permutation(len(df))] #chookw
        raw_data = df1.values #chookw
...
def main():
    # pass
    dp = data_preprocess(file_path = 'C:\\project\\STALSTM\\data\\dataset\\sample_t+1.csv',train_per = 0.8, vali_per = 0.0, in_dim = 96)
