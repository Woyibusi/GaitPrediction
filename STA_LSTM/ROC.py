...
net = torch.load('C:\\projects\\STALSTM\\models\\adult_train_test_split_oct_2024_finetune.pth')
...
def test():
    ...
    # out = out.detach().numpy()
    # groundtruths = groundtruths.detach().numpy()
    y_prob = [inner_list[1] for inner_list in predictions]
    y_test = [inner_list[1] for inner_list in test_groundtruths]
    ...
    plt.plot(fpr, tpr, color='blue', lw=2, label='Area Under the Curve = %0.3f' % roc_auc)
    plt.plot([0, 1], [0, 1], color='gray', linestyle='--')
    plt.xlim([0.0, 1.0])
    plt.ylim([0.0, 1.05])
    plt.xlabel('Specificity')
    plt.ylabel('Sensitivity')
    plt.title('Mild Pain vs No Pain & Significant Pain')
   ...
   # np.random.seed(0)
    ...
    return acc, err, predictions, test_groundtruths
...
def main():
    ...
    # 记录测试开始的时间
    test_start = time.time()
    acc, average_error, predictions, test_groundtruths = test()
