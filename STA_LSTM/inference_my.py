...
dp = data_preprocess(file_path = 'C:\\project\\STALSTM\\dataset\\KKH_49to100_preprocessed2keypoints.csv', train_per = TRAIN_PER, vali_per = VALI_PER, in_dim = IN_DIM)
...
'''****************************model prepration*******************************''' 
net = torch.load('C:\\project\\STALSTM\\models\\SGH_26to100_b2_e100.pth')
...
def test():
    ...
    dat = correct_pred.numpy()
    file = open("pred_result_mild_b2_200.csv", 'w')
    writer = csv.writer(file)
    writer.writerow(dat)
    file.close()
    
    dat = tf.argmax(predictions,1)
    dat = tf.cast(dat, tf.float32)
    file = open("pred_mild_max_b2_200.csv", 'w')
    writer = csv.writer(file)
    writer.writerow(dat.numpy())
    file.close()
